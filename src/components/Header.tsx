    ...
    <>
      <header className="sticky-header fixed top-0 left-0 w-full z-50 px-12 py-6 flex justify-between items-center">
        <div className="logo flex items-center">
          <span className="text-[#fff] text-2xl font-bold">MICA</span>
          <span className="text-[#fff] text-2xl">EL</span>
        </div>
        ...
        >
          <div className="flex flex-col justify-between w-6 h-5">
            <span className={`h-0.5 w-full bg-[#fff] transition-all duration-300 ${menuOpen ? 'rotate-45 translate-y-2' : ''}`}></span>
            <span className={`h-0.5 w-full bg-[#fff] transition-all duration-300 ${menuOpen ? 'opacity-0' : ''}`}></span>
            <span className={`h-0.5 w-full bg-[#fff] transition-all duration-300 ${menuOpen ? '-rotate-45 -translate-y-2' : ''}`}></span>
          </div>
        </button>
